...
class HueTab:
    ...
    def _connect_loop(self):
        """Verbinde zu Hue Bridge mit robusten Error Handling."""
        from phue import Bridge, PhueRegistrationException
        
        while self.alive:
            try:
                if self.bridge is None:
                    print(f"[HUE] Versuche zu verbinden: {HUE_BRIDGE_IP}")
                    self.bridge = Bridge(HUE_BRIDGE_IP)
                    self.bridge.connect()
                    self._ui_set(self.status_var, "✓ Verbunden")
                    self.root.after(0, self._refresh_scenes_ui)
                    print("[HUE] Verbindung erfolgreich")
                    
                # Background refresh (alle 30s)
                time.sleep(30)
                if self.alive and self.bridge:
                    try:
                        self.root.after(0, self._refresh_scenes_ui)
                ...
                self._ui_set(self.status_var, "⚠ KNOPF AUF BRIDGE DRÜCKEN!")
                time.sleep(5)
            except ConnectionError as e:
                self._ui_set(self.status_var, f"❌ Keine Verbindung ({HUE_BRIDGE_IP})")
                self.bridge = None
                time.sleep(10)
            except Exception as e:
                print(f"[HUE] Connection error: {e}")
                self._ui_set(self.status_var, f"⚠ Fehler: {str(e)[:30]}")
                self.bridge = None
                time.sleep(10)

    def _refresh_scenes_ui(self):
        """Zeigt Szenen statt einzelner Lampen (MAX 10 pro Raum)."""
        try:
            ...
            if not self.bridge:
                self._show_error_frame("Keine Bridge Verbindung")
                return
            
            scenes = self.bridge.get_scene()
            groups = self.bridge.get_group()
            group_names = {gid: g.get("name", f"Raum {gid}") for gid, g in groups.items()}

            if not scenes:
                self._show_error_frame("Keine Szenen gefunden")
                return
            ...
            for scene_id, scene in scenes.items():
                try:
                    ...
                    print(f"[HUE] Scene parse error: {e}")
                    continue
            
            # Sortiere und begrenze auf TOP 10 pro Raum
            scene_list = []
            for group_name in sorted(scenes_by_group.keys()):
                scene_items = sorted(scenes_by_group[group_name], key=lambda x: x[0])[:10]
                for name, scene_id in scene_items:
            ...
            if not scene_list:
                self._show_error_frame("Keine Szenen nach Filter")
                return
            ...
            print(f"[HUE] Refresh error: {e}")
            self._show_error_frame(f"Fehler: {str(e)[:40]}")
    
    def _show_error_frame(self, message: str):
        """Zeige Error Message im Scroll Frame."""
        for widget in self.scroll_frame.winfo_children():
            widget.destroy()
        tk.Label(
            self.scroll_frame,
