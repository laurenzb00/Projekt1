...
class StatusBar(tk.Frame):
    ...
    def __init__(self, parent: tk.Widget, on_exit=None, on_toggle_fullscreen=None):
        ...
        self.window_btn = RoundedButton(inner, text="⊡", command=on_toggle_fullscreen, bg=COLOR_BORDER, fg=COLOR_TEXT, radius=6, padding=(4, 1), font_size=9)
        self.window_btn.grid(row=0, column=2, sticky="e", padx=(2, 2), pady=1)

        # Kleiner Exit-Button
        self.exit_btn = RoundedButton(inner, text="⏻", command=on_exit, bg=COLOR_DANGER, fg="#fff", radius=6, padding=(4, 1), font_size=9)
        self.exit_btn.grid(row=0, column=3, sticky="e", padx=(2, 4), pady=1)
